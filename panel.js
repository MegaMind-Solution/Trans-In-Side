...
window.addEventListener("drop", (e) => {
  ...
  if (text && text.trim() !== "")
    chrome.storage.local.set({ lastQuery: text.trim() });
});
...
function setupToggle(elementId, storageKey, isTheme = false) {
  ...
  chrome.storage.sync.get([storageKey], (data) => {
    if (isTheme) {
      const isDark = data[storageKey] === "dark";
      el.checked = isDark;
      applyTheme(isDark);
    } else {
  ...
  el.addEventListener("change", (e) => {
    ...
    if (isTheme) {
      chrome.storage.sync.set({ theme: isChecked ? "dark" : "light" });
    } else {
      chrome.storage.sync.set({ tooltipEnabled: isChecked });
    }
  });
}
...
chrome.storage.onChanged.addListener((changes, area) => {
  if (area === "sync") {
    if (changes.theme) {
      ...
      applyTheme(isDark);
      ["themeToggle", "themeTogglePanel"].forEach((id) => {
        const el = document.getElementById(id);
        if (el) el.checked = isDark;
      });
    }
    if (changes.tooltipEnabled) {
      const enabled = changes.tooltipEnabled.newValue;
      ["tooltipToggle", "tooltipTogglePanel"].forEach((id) => {
        const el = document.getElementById(id);
...
setupToggle("tooltipToggle", "tooltipEnabled", false);
setupToggle("tooltipTogglePanel", "tooltipEnabled", false);
...
function setupDropdown(id, isSource, customOptions = null) {
  ...
  if (customOptions) {
    ...
    optionsHtml = isSource
      ? `<li title="Detect Automaticly" data-val="auto">Detect Language</li>`
      : "";
...
function saveToHistory(text, targetLangCode) {
  ...
  chrome.storage.local.get(["translationHistory"], (data) => {
    ...
    const existingIdx = history.findIndex(
      (h) => h.text === text && h.targetLangCode === targetLangCode,
    );
      ...
      history.forEach((item) => {
        ...
        li.querySelector(".history-text-wrap").addEventListener("click", () => {
          chrome.storage.sync.set({ targetLang: item.targetLang }, () => {
            chrome.storage.local.set({ lastQuery: item.text }, () => {
              historyOverlay.classList.remove("show");
            });
          });
        });
